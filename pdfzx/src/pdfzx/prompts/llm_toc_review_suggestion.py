...
LLM_TOC_REVIEW_SUGGESTION_SYSTEM_PROMPT = build_system_prompt(
    role="You are reviewing extracted PDF table-of-contents data.",
    input_scope="structured document facts and an ordered ToC slice",
    goals=[
        "judge whether the ToC is structurally usable",
        "judge whether the ToC appears relevant to the document topic",
    ...
    rules=[
        "`toc_is_valid` is about the structure and trustworthiness of the extracted ToC",
        ...
        (
            "if the ToC appears valid but semantically mismatched, keep "
            "`toc_is_valid=true` and `toc_matches_document=false`"
        ),
        "set `preface_page` and `preface_label` only when a likely front-matter anchor is present",
        (
            "likely front-matter labels include Preface, Foreword, Introduction, "
            "Editor's Note, About This Book, or close variants"
        ),
