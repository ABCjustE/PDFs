...
def test_scan_config_rejects_missing_root(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="PDFZX_ROOT does not exist"):
...
def test_scan_config_rejects_non_directory_root(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="PDFZX_ROOT is not a directory"):
...
def test_scan_config_rejects_missing_db_parent(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="PDFZX_DB parent directory does not exist"):
...
def test_scan_config_rejects_non_directory_db_parent(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="PDFZX_DB parent path is not a directory"):
        ScanConfig(root_path=tmp_path, db_path=db_path)


def test_get_config_requires_pdfzx_root(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFZX_ROOT", raising=False)
    monkeypatch.delenv("PDFZX_DB", raising=False)

    with pytest.raises(ValueError, match="PDFZX_ROOT environment variable is required"):
        get_config()
...
def test_get_config_reads_environment_each_call(
    ...
    monkeypatch.setenv("PDFZX_ROOT", str(first_root))
    monkeypatch.setenv("PDFZX_DB", str(first_db))
    first_config = get_config()

    monkeypatch.setenv("PDFZX_ROOT", str(second_root))
    monkeypatch.setenv("PDFZX_DB", str(second_db))
